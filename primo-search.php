...
//function to call JSON feeds and output search results
function PrimoSearch($primo_scope, $primo_tab, $query, $limit,$campus,$alma,$count) {
	if($limit !='') { //Limit to a specific resource type, see limit options below
	...
	foreach($bibrecords as $bibrecord){ //for each bib record in the search results do the following
	   ...
	   $recordid = is_array($recordidraw) ? $recordidraw[0] : $recordidraw; //Is there more than one record id? Get the first one[0] or the only one
	   ...
	   //$creationdate = (strlen($creationdateraw) == 4 ? $creationdateraw : substr($creationdateraw, -4)); // If creation date has more than four characters get the last four characters containing the year -doesn't work because of Hathi Trust formatting
	...
	echo '</ol>';
	echo '<a href="https://'.$campus.'-primo.hosted.exlibrisgroup.com/primo-explore/search?query=any,contains,'.$query.',AND'.$weblimits.'&tab='.$primo_tab.'&search_scope='.$primo_scope.'&sortby=rank&vid='.$alma.'&lang=en_US&mode=advanced&offset=0">See All <strong>';
		if($limit == 'reference_entrys') //Fixed spelling
			echo 'Reference Entries';
		else
			echo ucwords(str_replace('_', ' ', $limit)); //replace _ with a space, then uppercase first words
	echo '</strong></a>';
	//echo '<br />'.$url; // show JSON URL for debugging
}
